	...
	Context("packing packets", func() {
		...
		Context("packing 0-RTT packets", func() {
			BeforeEach(func() {
				...
				sealingManager.EXPECT().Get1RTTSealer().Return(nil, errors.New("not yet available")).AnyTimes()
			...
			It("packs a 0-RTT packet", func() {
				...
				cf := &wire.MaxDataFrame{ByteOffset: 0x1337}
				framer.EXPECT().AppendControlFrames(nil, gomock.Any()).DoAndReturn(func(frames []wire.Frame, _ protocol.ByteCount) ([]wire.Frame, protocol.ByteCount) {
					return append(frames, cf), cf.Length(packer.version)
				})
				framer.EXPECT().AppendStreamFrames(gomock.Any(), gomock.Any()).DoAndReturn(func(frames []wire.Frame, _ protocol.ByteCount) ([]wire.Frame, protocol.ByteCount) {
					return frames, 0
				})
				p, err := packer.PackPacket()
				Expect(p).ToNot(BeNil())
